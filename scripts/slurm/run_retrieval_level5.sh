...
LEVEL5_PHASE1_DATASET_NAME="${LEVEL5_PHASE1_DATASET_NAME:-l1000_phase1_level5}"
LEVEL5_PHASE2_DATASET_NAME="${LEVEL5_PHASE2_DATASET_NAME:-l1000_phase2_level5}"

LEVEL5_PHASE1_PATH="${LEVEL5_PHASE1_PATH:-/lustre/groups/ml01/workspace/olga.novitskaia/data_updated/l1000_phase1/level5/level5_phase1_not_filtered.h5ad}"
LEVEL5_PHASE2_PATH="${LEVEL5_PHASE2_PATH:-/lustre/groups/ml01/workspace/olga.novitskaia/data_updated/l1000_phase2/level5/level5_phase2_not_filtered.h5ad}"

QUERY_DATASETS="${QUERY_DATASETS:-${LEVEL5_PHASE1_DATASET_NAME},${LEVEL5_PHASE2_DATASET_NAME},sciplex,tahoe}"
DB_DATASETS="${DB_DATASETS:-${LEVEL5_PHASE1_DATASET_NAME},${LEVEL5_PHASE2_DATASET_NAME},sciplex,tahoe}"
...
if [ -n "${DATASET_PATH_ARGS}" ]; then
    DATASET_PATH_ARGS="--dataset-path ${LEVEL5_PHASE1_DATASET_NAME}=${LEVEL5_PHASE1_PATH} --dataset-path ${LEVEL5_PHASE2_DATASET_NAME}=${LEVEL5_PHASE2_PATH} ${DATASET_PATH_ARGS}"
else
    DATASET_PATH_ARGS="--dataset-path ${LEVEL5_PHASE1_DATASET_NAME}=${LEVEL5_PHASE1_PATH} --dataset-path ${LEVEL5_PHASE2_DATASET_NAME}=${LEVEL5_PHASE2_PATH}"
fi

# Keep all interactions with level5 datasets, skip direct sciplex<->tahoe pairs.
EXCLUDE_PAIRS="${EXCLUDE_PAIRS:-sciplex:tahoe,tahoe:sciplex}"

OUTPUT_DIR="${OUTPUT_DIR:-./results/${SUBDIR}/level5}"
TASK_OUTPUT_DIR="${TASK_OUTPUT_DIR:-${OUTPUT_DIR}/tasks}"
LOGS_DIR="${LOGS_DIR:-./logs/retrieval/${SUBDIR}/level5}"
OUTPUT_PREFIX="${OUTPUT_PREFIX:-cross_dataset_retrieval_level5_parallel${SUFFIX}}"
...
sbatch -W -J retrieval_level5_precompute \
    -t "${PREP_TIME}" \
    -n 1 \
    --qos="${QOS}" \
    --partition="${PARTITION}" \
    ...
    --mem="${PREP_MEM}" \
    -e "${LOGS_DIR}/retrieval_level5_precompute.%j.err" \
    -o "${LOGS_DIR}/retrieval_level5_precompute.%j.out" \
    --wrap="${PRECOMPUTE_CMD}"
...
sbatch -W -J retrieval_level5_task \
    -t "${TASK_TIME}" \
    -n 1 \
    --array=1-"${N_TASKS}" \
    --qos="${QOS}" \
    ...
    --cpus-per-task="${CPUS_PER_TASK}" \
    --mem="${TASK_MEM}" \
    -e "${LOGS_DIR}/retrieval_level5_task.%A_%a.err" \
    -o "${LOGS_DIR}/retrieval_level5_task.%A_%a.out" \
    --wrap="${RUN_TASK_CMD}"

echo "> Stage 3/3: Merge task outputs"
sbatch -W -J retrieval_level5_merge \
    -t "${MERGE_TIME}" \
    -n 1 \
    --qos="${QOS}" \
    --partition="${PARTITION}" \
    --cpus-per-task="${CPUS_PER_TASK}" \
    --mem="${MERGE_MEM}" \
    -e "${LOGS_DIR}/retrieval_level5_merge.%j.err" \
    -o "${LOGS_DIR}/retrieval_level5_merge.%j.out" \
    --wrap="${MERGE_CMD}"
